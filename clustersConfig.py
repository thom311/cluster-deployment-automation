    ...
    def system_check(self) -> None:
        ...
        if self.rebuild_dpu_operators_images:
            if not common.podman_pull(
                ...
                raise ValueError(
                    f"\"{self.yamlpath}.rebuild_dpu_operators_images\": Building dpu-operator requires permissions to fetch. Get a token from https://oauth-openshift.apps.ci.l2s4.p1.openshiftapps.com/oauth/token/request and test with `podman login registry.ci.openshift.org`. Copy the authfile to \"/root/config.json\""
                )
...
class HostConfig(kcommon.StructParseBaseNamed):
    ...
    def serialize(self) -> dict[str, Any]:
        ...
        kcommon.dict_add_optional(extra_2, "password", self.password)
        if self.is_generated:
            extra_3["is_generated"] = True
        return {
            **super().serialize(),
            **extra_1,
...
class ClusterConfig(kcommon.StructParseBaseNamed):
    ...
    def serialize(self, *, with_generated_hosts: bool = True) -> dict[str, Any]:
        extra_1: dict[str, Any] = {}
        ...
        return {
            ...
            **extra_1,
            ...
            "hosts": [h.serialize() for h in self.hosts.values() if with_generated_hosts or not h.is_generated],
        ...
        *,
        ...
        if kind == "iso":
            ...
            if master.dpu_kind not in VALID_DPU_KIND_DPU:
                raise ValueError(f"\"{master.yamlpath}.dpu_kind\": for a cluster kind {repr(kind)} the master has an unexpected dpu_kind {repr(master.dpu_kind)}")
...
class MainConfig(kcommon.StructParseBase):
    ...
    def serialize(self, *, with_generated_hosts: bool = True) -> dict[str, Any]:
        return {
            "clusters": [n.serialize(with_generated_hosts=with_generated_hosts) for n in self.clusters],
        }

    def serialize_json(self, *, with_generated_hosts: bool = True) -> str:
        return json.dumps(self.serialize(with_generated_hosts=with_generated_hosts))
