        ...
        *,
        ...
        with pctx.with_strdict() as varg:
            ...
            if name not in ExtraConfigArgs.get_extra_configs():
                ...
                raise pctx.value_error(f"invalid value {repr(name)} must be one of {repr(list(valid_configs))}", key="name")
...
class NodeConfig(ClusterConfigStructParseBase):
    ...
    def serialize(self, *, show_secrets: bool = False) -> dict[str, Any]:
        ...
        kcommon.dict_add_optional(extra_2, "host_side_bmc", self.host_side_bmc)
        ...
        *,
        ...
        with pctx.with_strdict() as varg:
            ...
            if mac_random is None:
                ...
                if not val_valid:
                    raise pctx.value_error(f"invalid MAC address {repr(mac_random)}", key="mac_random")
            ...
            if mac_explicit is not None:
                ...
                if not val_valid:
                    raise pctx.value_error(f"invalid MAC address {repr(mac_explicit)}", key="mac")
            ...
            if bmc is None:
                if kind in ("physical", "bf", "ipu"):
                    raise pctx.value_error(f"BMC is mandatory for node kind {repr(kind)}", key="bmc")
            ...
            host_side_bmc = kcommon.structparse_pop_str(
            ...
            if ip is not None:
                try:
                    ...
                    raise pctx.value_error(f"invalid IP address {repr(ip)}", key="ip") from None
            ...
            if image_path is None:
                ...
                if not has_image_path:
                    raise pctx.value_error(f"not allowed for cluster kind {repr(cluster_kind)}", key="image_path") from None
        ...
        if ip is None:
            if kind in ("vm",):
                raise pctx.value_error(f"mandatory for node of kind {repr(kind)}", key="ip")
            if kind == "marvell-dpu" and cluster_kind == "iso":
                raise pctx.value_error(f"mandatory for node of kind {repr(kind)} and cluster kind {repr(cluster_kind)}", key="ip")
        ...
        *,
        ...
        with pctx.with_strdict() as varg:
            ...
            if network_api_port is not None:
                ...
                if not val_valid:
                    msg = f"{repr(network_api_port)} is not a valid interface name"
                    if network_api_port_is_default:
                        msg = f"default {msg}"
                    raise pctx.value_error(msg, key="network_api_port")
        ...
        *,
        ...
        with pctx.with_strdict() as varg:
            ...
            if version is not None and _parse_ocp_version(version) is None:
                raise pctx.value_error(f"version {repr(version)} does not look like a valid OCP version", key="version")
        ...
        for extra_config in pconfigs:
            if extra_config.name in ("ovn_custom", "microshift", "dpu_operator_dpu"):
                if len(masters) < 1:
                    raise pctx.value_error(f"{extra_config.config_type} {repr(extra_config.name)} (\"{extra_config.yamlpath}\") requires masters", key="masters")
            if extra_config.name in ("ovn_custom", "microshift", "dpu_operator_dpu"):
                master = next(iter(masters.values()))
                if master.ip is None:
            ...
            if extra_config.name in ("microshift",):
                if network_api_port is None:
                    raise pctx.value_error(f"the parameter is mandatory with {extra_config.config_type} {repr(extra_config.name)} (\"{extra_config.yamlpath}\")", key="network_api_port")
        ...
        *,
        ...
        for node in all_nodes:
            if node.ip is not None and not common.ip_range_contains(real_ip_range, node.ip):
                raise pctx.value_error(f"the node {repr(node.name)} (\"{node.yamlpath}\") with IP {node.ip} is not in the supplied IP range {real_ip_range}", key="ip_range")
...
class MainConfig(kcommon.StructParseBase):
        ...
        *,
        ...
        with pctx.with_strdict() as varg:
            clusters = kcommon.structparse_pop_objlist(
                ...
                construct=lambda pctx2: ClusterConfig.parse(
                    pctx2,
                    basedir=basedir,
                    rnd_seed=rnd_seed,
                ),
    ...
    def _apply_jinja(
        ...
        *,
        ...
        ci = clusterInfo.ClusterInfoLoader(
            current_host=cluster_name,
            delegate_loader=cluster_info_loader,
        )
        ...
        return result, ci.current_host if ci.is_loaded else None
    ...
    def load(
        ...
        *,
        ...
        try:
            cc = MainConfig.parse(
                StructParseParseContext(yamldata),
                yamlfile=filename,
                basedir=basedir,
                rnd_seed=rnd_seed,
