...
How to and examples:
    ...
    A.1) run local container registry:
      ...
      $ CDA_LOG_LEVEL=debug \\
        python -c 'if 1:
                import imageRegistry, host

                ireg = imageRegistry.ImageRegistry(host.LocalHost())
                ireg.ensure_running(delete_all=False)
                print(f"registry: {ireg.url()}")
            '
    ...
    A.2) or, run local container registry and trust the CA certificate in openshift:
      ...
      $ KUBECONFIG=/root/kubeconfig.nicmodecluster \\
        CDA_LOG_LEVEL=debug \\
        python -c 'if 1:
                import imageRegistry, host, k8sClient, os

                lsh = host.LocalHost()
                client = k8sClient.K8sClient(os.getenv("KUBECONFIG"))
                ireg = imageRegistry.ImageRegistry(lsh)
                ireg.start_image_registry(client)
                print("")
                print(f"REGISTRY=\\"{ireg.url()}\\"")
                print(f"PODMAN_PUSH_ARGS=\\"--cert-dir={ireg.certificate_path()}\\"")
            '
...
build() {
    ...
    if [ "$REBUILD" != 0 -o -z "$(podman images -q "$FULL_TAG")" ] ; then
