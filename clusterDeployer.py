...
class ClusterDeployer:
    ...
    steps: tuple[str, ...]
    bf_connections: dict[str, host.Host]
    ...
    def _deploy_cluster(self, duration: collections.abc.Mapping[str, timer.StopWatch]) -> None:
        ...
        if self._cc.kind == "iso":
            ...
            with duration[MASTERS_STEP]:
                if master.kind == "ipu":
                    node = ipu.IPUClusterNode(master, self._cc.get_external_port(), unwrap(self._cc.cluster_config.network_api_port))
    ...
    def _start_node(self, infra_env: str, node: ClusterNode, master: bool) -> bool:
        image = os.path.join(os.path.dirname(unwrap(node.config.image_path)), f"{infra_env}.iso")
